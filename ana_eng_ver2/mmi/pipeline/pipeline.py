...
def run(cfg: PipelineConfig, verbose: bool = True) -> Scene:
    ...
    log(f"1/6 ingest        : {cfg.get_videos()[0]}{' + ' + str(len(cfg.get_videos())-1) + ' more views' if cfg.is_multi_view() else ''}")
    ing = ingest.run(cfg)
    primary = ing.views[0]
    log(f"      -> {primary.frame_paths} frames @ ~{primary.sampled_fps:.1f} fps{' x ' + str(ing.view_count) + ' views' if ing.is_multi_view else ''}")
    ...
    scene = assemble.run(cfg, rec, seg, trk, fps=max(1, round(primary.sampled_fps)))
