...
def main() -> int:
    ...
    safe_root = (repo_root / "data" / "samples").resolve()
    requested_out = args.out
    candidate_out = (safe_root / requested_out).resolve() if not requested_out.is_absolute() else requested_out.resolve()
    try:
        candidate_out.relative_to(safe_root)
    except ValueError:
        print(f"Refusing to write outside {safe_root}: {requested_out}", file=sys.stderr)
        return 2
    out = scene.save(candidate_out)
