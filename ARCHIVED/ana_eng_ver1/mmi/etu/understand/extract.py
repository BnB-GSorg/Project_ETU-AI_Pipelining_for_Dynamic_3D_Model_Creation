...
def extract(
    ...
    if not frames:
        return FeatureGraph()
    # even time sampling
    picked = frames if len(frames) <= max_images else \
        [frames[round(i * (len(frames) - 1) / (max_images - 1))] for i in range(max_images)]
    ...
    return FeatureGraph.from_dict(_parse_json(raw))
