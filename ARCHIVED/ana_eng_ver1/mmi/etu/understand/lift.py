...
def _track(obj: FeatureObject, duration: int) -> list[Keyframe]:
    """Build the object's pose track, with opacity driving its lifetime.

    An object is only "alive" between its first and last observed state. Before
    it first appears and after it last appears (e.g. it merged into another or
    left the frame), we emit opacity-0 keyframes so the viewer hides it instead
    of leaving a stale primitive sitting on screen. Per-state opacity is carried
    through too, so a fading/merging object fades out faithfully.
    """
    states = sorted(obj.timeline, key=lambda s: s.t)
    if not states:
        return [Keyframe(0, _to_scene(0.5, 0.5, obj.depth))]

    first_t = min(states[0].t, duration - 1)
    last_t = min(states[-1].t, duration - 1)
    # Only annotate opacity when lifetime is non-trivial (born late, dies early,
    # or any explicit fade) — keep tracks clean for always-on objects.
    needs_lifetime = first_t > 0 or last_t < duration - 1 or any(s.opacity < 1.0 for s in states)

    kfs: list[Keyframe] = []
    if needs_lifetime and first_t > 0:
        # hidden up to the frame before it appears
        kfs.append(Keyframe(t=0, position=_to_scene(states[0].x, states[0].y, obj.depth), opacity=0.0))

    for s in states:
        pos = _to_scene(s.x, s.y, obj.depth)
        k = max(0.05, s.size) * 6.0           # size fraction -> scene scale
        kfs.append(Keyframe(
            t=min(s.t, duration - 1), position=pos, scale=[k, k, k],
            opacity=(s.opacity if needs_lifetime else None)))

    if needs_lifetime and last_t < duration - 1:
        # disappears the frame after its last sighting
        last = states[-1]
        kfs.append(Keyframe(t=last_t + 1, position=_to_scene(last.x, last.y, obj.depth), opacity=0.0))
...
def lift(fg: FeatureGraph, title: str | None = None) -> Scene:
    ...
    return Scene(
        ...
        fps=max(1, fg.fps), duration_frames=duration, objects=objects, layers=layers,
