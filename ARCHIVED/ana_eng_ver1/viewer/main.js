...
// per-object track lookup: latest keyframe with t <= frame (step-hold).
...
function applyFrame(t) {
  ...
  for (const o of objects) {
    const k = transformAt(o.track, frame);
    o.mesh.position.fromArray(k.position);
    if (k.quaternion) o.mesh.quaternion.fromArray(k.quaternion);
    if (k.scale) o.mesh.scale.fromArray(k.scale);
